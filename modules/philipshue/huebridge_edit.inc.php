...
*/
  ...
  if ($this->tab=='data') {
   ...
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM huedevices WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM huedevices WHERE bridgeid='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('huedevices', $properties[$i]);
     }
   }
